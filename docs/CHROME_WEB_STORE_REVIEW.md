...
本扩展主要连接企业本地运行的招聘后台。为了让审核员无需访问企业内网、无需 BOSS 账号，也能验证全部核心交互，V0.7.1 内置了完全离线的“商店审核模式”。

测试步骤：

1. 安装扩展，点击工具栏图标打开独立工具窗口。
2. 在首次连接页点击“进入商店审核演示”，无需填写邮箱或访问码。
3. 在“任务”页领取任一合成任务。可以复制搜索词；点击“打开演示页”会打开扩展内置的合成长简历。
4. 保持演示页为活动页并点击一次工具栏中的扩展图标，工具窗口会自动聚焦并取得 Chrome 对该演示标签页的临时捕获授权。点击“分析当前演示页”，确认截图提示后可看到合成候选人的评分、可见证据和未知项。
5. 点击“生成招呼语草稿”，可验证草稿生成和复制；扩展不会自动发送消息。
6. 切换到“扫描简历”，点击“开始扫描”。扩展会按标签页 ID 定向捕获原始 Chrome 窗口中的当前演示页，不会弹出屏幕共享选择器；随后每滚动到一个需要保留的位置，点击工具窗口中的“截取当前帧”，再点击“完成扫描并识别”。工具窗口会显示关键帧数量和本地合成识别结果。
...
This extension normally connects to an employer-operated recruitment backend on localhost. Version 0.7.1 includes a fully offline Chrome Web Store Review Mode so reviewers can verify every core interaction without access to the employer network, a BOSS account, credentials, or real candidate data.

Review steps:

1. Install the extension and click its toolbar icon to open the standalone workspace window.
...
4. Keep the synthetic page active and click the extension toolbar icon once. The workspace window is focused and Chrome grants temporary capture access to that demo tab. Click “分析当前演示页”; after confirming the visible-page capture, the extension shows a synthetic score, visible evidence, and unknown items.
5. Click “生成招呼语草稿” to verify greeting draft generation and copy. The extension never sends messages automatically.
6. Open the Resume Scan tab and click “开始扫描”. The extension targets the current demo tab in the original Chrome window by tab ID and does not open a screen-sharing picker. At each scroll position you want to capture, click “截取当前帧” in the workspace window, then click “完成扫描并识别”. The workspace window displays the keyframe count and a local synthetic extraction result.
