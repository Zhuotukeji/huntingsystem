...
export async function POST(request: Request) {
  try {
    ...
    const task = input.type === "organizations" ? await discoverOrganizations(input.campaignId) : input.type === "people" ? await discoverPeople(input.campaignId) : input.type === "insighttracker" ? await createInsightTrackerTask(input.campaignId) : null;
