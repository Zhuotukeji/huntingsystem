...
export default function LearningPage() {
  ...
  return <>
    ...
    <div className="metrics-grid">
      <Metric icon={BrainCircuit} label="待学习简历" value={overview.pendingResumes} detail={"简历库共 " + overview.resumes + " 份"} />
      <Metric icon={GitBranch} label="图谱关系" value={overview.graphEdges} detail="任职和技能证据边" tone="blue" />
      <Metric icon={SearchCheck} label="开放搜索任务" value={overview.openSearchTasks} detail="等待 HR 在 BOSS 手工执行" tone="amber" />
      <Metric icon={Clock3} label="反馈样本" value={overview.feedbackSamples} detail="用于小步调整搜索信号" tone="purple" />
    </div>
    <section className="section"><div className="section-head"><div><h3>运行学习</h3><p>增量学习只处理新数据；重建图谱会重新分析该画像的全部简历</p></div></div><LearningControls campaigns={campaigns} /></section>
