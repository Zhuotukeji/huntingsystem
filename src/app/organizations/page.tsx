...
export default async function OrganizationsPage({ searchParams }: { searchParams: Promise<{ q?: string | string[]; campaignId?: string | string[] }> }) {
  ...
  return <>
    ...
    {organizations.length ? <OrganizationWorkbench key={initialQuery} organizations={organizations} initialQuery={initialQuery} canManage={hasPermission(user, "organizations.manage")} /> : <div className="panel"><div className="empty-state"><Building2 size={28} /><strong>还没有从简历中发现公司</strong><p>先在简历库新增授权简历，再到 AI 学习中心运行增量学习。</p></div></div>}
  </>;
}
