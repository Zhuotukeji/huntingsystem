...
export default function CampaignsPage() {
  ...
  return <>
    <PageIntro eyebrow="Campaigns" title="寻访战役" description="每个战役都从业务结果和成功画像开始，独立沉淀公司地图、人选队列与反馈。" actions={<CampaignCreate />} />
    <div className="campaign-grid">
      {campaigns.map((campaign) => {
        ...
        return <Link className="campaign-card" href={`/campaigns/${campaign.id}`} key={campaign.id}>
          ...
          <div className="campaign-card-foot"><span>{orgCount} 家公司 · {personCount} 名人选</span><ArrowRight size={15} /></div>
        </Link>;
      })}
    </div>
  </>;
