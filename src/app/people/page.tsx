...
export default async function PeoplePage({ searchParams }: { searchParams: Promise<{ q?: string | string[] }> }) {
  ...
  return <>
    ...
    {people.length ? <PersonWorkbench key={initialQuery} people={people} initialQuery={initialQuery} /> : <div className="panel"><div className="empty-state"><UserRoundSearch size={28} /><strong>还没有候选人</strong><p>请先在简历库新增授权简历并运行增量学习。</p></div></div>}
  </>;
}
