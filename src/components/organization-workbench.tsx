...
export function OrganizationWorkbench({ organizations, initialQuery = "", canManage = false }: { organizations: CampaignOrganization[]; initialQuery?: string; canManage?: boolean }) {
  ...
  return <>
    ...
    {filtered.length ? <div className="table-scroll"><table className="entity-table organization-table"><thead><tr><th></th><th>公司</th><th>简历人才</th><th>发现职位</th><th>市场 / 渠道</th><th>匹配 / 证据</th><th>状态</th><th>详情</th></tr></thead><tbody>{filtered.map((item) => <Fragment key={item.id}>
      ...
      {expanded === item.id ? <tr className="detail-row"><td colSpan={8}><EntityDetails item={item} onReview={review} canManage={canManage} /></td></tr> : null}
...
function EntityDetails({ item, onReview, canManage }: { item: CampaignOrganization; onReview: (status: ReviewStatus, ids: string[]) => Promise<void>; canManage: boolean }) {
  return <div className="organization-detail-grid">
    ...
    <section><h4><AlertCircle size={14} />待人工确认</h4><ul>{item.unknowns.length ? item.unknowns.map((unknown) => <li key={unknown}>{unknown}</li>) : <li>暂无待确认项</li>}</ul>{canManage ? <div className="detail-actions">{item.status !== "APPROVED" ? <button className="button small" onClick={() => onReview("APPROVED", [item.id])}>批准公司</button> : null}<button className="button small" onClick={() => onReview("WATCHLIST", [item.id])}>加入观察</button></div> : null}</section>
  </div>;
}
