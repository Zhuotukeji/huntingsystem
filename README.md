...
1. HR 在网页端导入授权简历，或由插件逐屏识别当前可见的 BOSS 简历详情。
...
- Chrome MV3 侧边栏插件 V0.3：端口自动探测、内部登录、任务领取/执行/反馈、结果页单屏判断、简历详情多屏采集和个性化招呼语草稿。
- 无 AI 降级：未配置 Sub2API 时可解析带明确字段的结构化文本，不会生成虚构公司或候选人。

## 合规边界

插件没有 content script，不读取 BOSS DOM、Cookie、密码或验证码，不自动翻页、滚动、打开候选人或发送消息。截图只在 HR 点击后截取当前可见标签页并发送至已配置的 Sub2API，后端不保存原图；多屏采集只在 Chrome 会话中暂存识别文本，完成后合并为永久简历。招呼语仅生成草稿，由 HR 审核后手工发送。
...
3. 加载仓库中的 `extension/` 目录，或下载并解压 `artifacts/hunting-extension-v0.3.0.zip` 后加载。
4. 点击扩展图标。插件会依次检测 `localhost:3010`、`localhost:3000`，登录后即可领取任务、回填反馈或逐屏采集 BOSS 简历。
...
测试覆盖初始空库、简历永久保留与去重、图谱构建、重建幂等、插件任务状态机、反馈学习边界、会话过期、截图来源、多屏重叠合并和密钥加密。
